...
{{
  "tasks": [
    {{
      ...
      "query": "建议使用的普通网络检索关键词",
      "tools": ["web", "arxiv"],
      "arxiv_query": "若工具中包含arxiv，提供纯英文的学术搜索词，否则留空"
    }}
  ]
}}
- "tools" 字段控制此任务使用的检索引擎：普通问题仅用 ["web"]，如果涉及学术前沿、科研原理等强文献需求，请务必包含 "arxiv"，如 ["web", "arxiv"]。
- "arxiv_query" 必须是纯正的英文关键词或短语，用于直接查询 ArXiv (不能包含中文)。
</FORMAT>
